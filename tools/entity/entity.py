...
try:
    ...
    get_progressive_embedding_service: Callable[..., Any] | None = None  # type: ignore[assignment]
...
class EntityManager(ToolBase):
    ...
    async def create_entity(
        ...
        if include_relations:
            logger.debug("include_relations requested during create; fetch relationships via read() to avoid client issues")
    ...
    async def _generate_embedding_async(self, entity_type: str, entity_data: dict[str, Any]):
        ...
        try:
            ...
            logger.warning(
                "Failed to generate embedding for %s %s: %s",
                entity_type,
                entity_data.get("id"),
                e
            )
...
async def _handle_create_operation(
    ...
    if batch:
        tasks = [
            entity_manager.create_entity(entity_type, item, include_relations)
            for item in batch
        ]
...
async def _handle_read_operation(
    ...
    t_op_start = time.time()
    read_result: dict[str, Any] | None = await entity_manager.read_entity(
        entity_type, identifier, include_relations
    )
    ...
    if read_result is None:
        ...
        if "suggestions" in suggestions_result:
            ...
            error_response["hint"] = (
                "Did you mean one of these? Use the exact ID or name from suggestions."
            )
...
async def _handle_update_operation(
    ...
    result = await entity_manager.update_entity(
        entity_type, identifier, payload, include_relations
    )
...
async def _handle_list_operation(
    ...
    t_op_start = time.time()
    list_results: list[dict[str, Any]] = await entity_manager.list_entities(
