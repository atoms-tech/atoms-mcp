...
class SessionMiddleware(BaseHTTPMiddleware):
    ...
    async def _handle_jwt_auth(self, jwt_token: str) -> Optional[str]:
        ...
        try:
            ...
            # Decode JWT to get user_id (don't validate - it was already validated by AuthKit)
            # The JWT is a Supabase JWT issued after WorkOS OAuth, so we can trust it
            import jwt as pyjwt
            try:
                # Decode to get claims (signature already verified by AuthKit OAuth flow)
                ...
                if decoded.get('role') != 'authenticated' and decoded.get('aud') != 'authenticated':
                    logger.warning(f"JWT has wrong role: {decoded.get('role')}")
                    return None
