"""Fixtures for integration workflow tests.
...
async def call_mcp(mcp_client_http):
    ...
    async def _call(tool_name: str, params: Dict[str, Any]) -> Tuple[Dict[str, Any], float]:
        ...
        try:
            ...
            import sys
            print(f"\n🔍 DEBUG call_mcp: tool={tool_name}, result={result}", file=sys.stderr)
            ...
            elapsed = time.time() - start
            import sys
            print(f"\n❌ ERROR call_mcp: tool={tool_name}, error={str(e)}", file=sys.stderr)
