...
class TestWorkspaceIntegration:
    ...
    async def test_workspace_project_lifecycle(self, workspace_client, cleanup_test_data):
        ...
        # Create a project
        result = await workspace_client.call(
            "create_project",
            {"name": "Integration Test Project", "description": "Project created during integration testing"},
        )
        ...
        if "result" in result and "project" in result["result"]:
            ...
            # Update project
            update_result = await workspace_client.call(
                "update_project", {"project_id": project_id, "updates": {"status": "in_progress"}}
            )
    ...
    async def test_workspace_and_entity_interaction(self, workspace_client, entity_client, cleanup_test_data):
        ...
        entity_result = await entity_client.call(
            "create",
            {
...
class TestWorkspaceBulkOperations:
    ...
    async def test_bulk_project_creation(self, workspace_client, cleanup_test_data):
        ...
        for name in project_names:
            result = await workspace_client.call(
                "create_project", {"name": name, "description": f"Bulk test project: {name}"}
            )
...
class TestWorkspaceErrorHandling:
    ...
    async def test_malformed_parameters(self, workspace_client):
        ...
        result = await workspace_client.call("create_project", {"invalid_param": "invalid_value"})
