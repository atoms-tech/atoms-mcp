...
async def test_complex_policy_check_under_threshold(
    ...
    can_access = await user_can_access_project("project-001", "user-test-001", mock_db_adapter)
...
async def test_bulk_operations_dont_n_plus_1(
    ...
    organizations = [{"id": f"org-{i:03d}"} for i in range(10)]
...
async def test_cached_policy_checks_are_faster(
    ...
    can_read_first = await validator.can_select(Tables.ORGANIZATIONS, {"id": "org-001"})
    ...
    can_read_second = await validator.can_select(Tables.ORGANIZATIONS, {"id": "org-002"})
    ...
    assert queries_for_second < first_query_count, "Second check should use cached data and make fewer queries"

    print(
        f"✓ Caching test: First={first_duration_ms:.2f}ms ({first_query_count} queries), "
...
async def test_concurrent_policy_evaluations(
    ...
    documents = [{"id": f"doc-{i:03d}", "project_id": "project-001"} for i in range(CONCURRENT_OPERATIONS)]

    # Measure concurrent execution
    start_time = time.perf_counter()

    # Execute all checks concurrently
    tasks = [policy.can_select(doc) for doc in documents]
    ...
    assert len(results) == CONCURRENT_OPERATIONS, f"Expected {CONCURRENT_OPERATIONS} results, got {len(results)}"
    ...
    print(
        f"✓ Concurrent checks ({CONCURRENT_OPERATIONS} operations): {duration_ms:.2f}ms "
...
async def test_policy_evaluation_scales_linearly(
    ...
    for batch_size in batch_sizes:
        projects = [{"id": f"project-{i:03d}"} for i in range(batch_size)]
    ...
    assert variance_factor < 2.0, f"Performance variance too high ({variance_factor:.2f}x), suggests non-linear scaling"

    # Print detailed timing analysis
    print("\n✓ Linear scaling test:")
    for timing in timings:
        print(
            f"  {timing['batch_size']:3d} checks: {timing['total_ms']:6.2f}ms total, {timing['avg_ms']:5.2f}ms average"
        )
