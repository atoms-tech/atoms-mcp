...
Reuses components from pheno-sdk's enhanced progress display.
"""
...
class ComprehensiveProgressDisplay:
    ...
    def _create_current_test_panel(self) -> Panel:
        ...
        with self._lock:
            ...
            # Determine if test is running slowly
            warning_threshold = 10.0  # Same as workflow manager
            is_slow = current_duration > warning_threshold
            ...
            if is_slow:
                info_text.append(f"{current_duration:.1f}s ⚠️", style="yellow bold")
            else:
            ...
            if is_slow:
                info_text.append("\n   ", style="dim")
                info_text.append("⚠️ Running longer than expected", style="yellow bold")

        # Use yellow border for slow tests
        border_style = "yellow" if is_slow else "magenta"
