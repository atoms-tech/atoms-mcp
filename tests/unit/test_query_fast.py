...
async def test_search_single_entity_project(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool", {"query_type": "search", "entities": ["project"], "search_term": "test"}
    )
...
async def test_search_single_entity_document(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool", {"query_type": "search", "entities": ["document"], "search_term": "requirements"}
    )
    ...
    assert isinstance(response_data, dict | list), f"Expected dict or list response, got {type(response_data).__name__}"
...
async def test_search_multi_entity_all(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool",
        {"query_type": "search", "entities": ["project", "document", "requirement"], "search_term": "test"},
    )
...
async def test_search_with_filters(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool",
        {"query_type": "search", "entities": ["project"], "conditions": {"status": "active"}, "search_term": ""},
    )
...
async def test_search_with_limit(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool", {"query_type": "search", "entities": ["document"], "search_term": "", "limit": 5}
    )
...
async def test_aggregate_single_entity_project(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool", {"query_type": "aggregate", "entities": ["project"], "conditions": {}}
    )
    ...
    assert isinstance(response, dict), f"Expected dict response for aggregate, got {type(response).__name__}"
...
async def test_aggregate_with_filters(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool",
        {"query_type": "aggregate", "entities": ["project", "requirement"], "conditions": {"status": "active"}},
    )
...
async def test_relationship_query(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool", {"query_type": "relationships", "entities": ["organization", "project"], "conditions": {}}
    )
...
async def test_relationship_with_filters(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool",
        {"query_type": "relationships", "entities": ["project", "document"], "conditions": {"status": "active"}},
    )
...
async def test_analyze_organization(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool", {"query_type": "analyze", "entities": ["organization"], "conditions": {}}
    )
...
async def test_analyze_project(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool", {"query_type": "analyze", "entities": ["project"], "conditions": {}}
    )

    assert result.get("success", True), f"Failed project analysis: {result.get('error')}"

    # Project analysis should include metrics
    assert "analysis" in result or "results" in result, f"Expected analysis results, got keys: {list(result.keys())}"
...
async def test_analyze_with_filters(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool",
        {"query_type": "analyze", "entities": ["project", "requirement"], "conditions": {"status": "active"}},
    )
...
async def test_search_performance(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool", {"query_type": "search", "entities": ["project", "document"], "search_term": "test"}
    )

    duration = time.time() - start_time

    assert result.get("success", True), f"Failed search operation: {result.get('error')}"
    assert duration < 3.0, f"Search operation took {duration:.2f}s, expected < 3.0s. Fast HTTP client should be faster."
...
async def test_aggregate_performance(authenticated_client):
    ...
    assert duration < 2.0, f"Aggregation took {duration:.2f}s, expected < 2.0s. Fast HTTP client should be faster."
...
async def test_empty_search_term(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool", {"query_type": "search", "entities": ["document"], "search_term": ""}
    )
...
async def test_invalid_entity_type(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool", {"query_type": "search", "entities": ["invalid_entity_type"], "search_term": "test"}
    )
...
async def test_rag_with_invalid_mode(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "query_tool",
        {"query_type": "rag_search", "entities": ["document"], "search_term": "test query", "rag_mode": "invalid_mode"},
    )
...
async def test_extreme_limit_values(authenticated_client):
    ...
    result_large = await authenticated_client.call_tool(
        "query_tool", {"query_type": "search", "entities": ["document"], "search_term": "", "limit": 999999}
    )
    ...
    result_zero = await authenticated_client.call_tool(
        "query_tool", {"query_type": "search", "entities": ["document"], "search_term": "", "limit": 0}
    )
