"""Basic entity operations tests: search, archive/restore, list, bulk, error handling.

This module contains straightforward, foundational entity operation tests:
- Basic search functionality (create, search, verify)
- Archive and restore operations (soft-delete, restore, list filtering)
- List filtering operations (filter by status, limit, pagination)
- Bulk operations (bulk_create, verify created items)
- Invalid operation error handling (invalid entity type, missing fields)

These tests were consolidated from test_working_extension.py to follow
canonical naming standard (concern-based, not semantic state-based).

Run with: pytest tests/unit/tools/test_entity_basic_operations.py -v
"""
...
class TestBasicOperations:
    ...
    async def test_basic_search_operations(self, call_mcp, test_organization, test_project):
        ...
        Verifies:
        - Document can be created with searchable content
        - Search operation returns matching documents
        - Created document appears in search results
        """
    ...
    async def test_archive_restore_operations(self, call_mcp, test_organization, test_project):
        ...
        Verifies:
        - Document can be archived (soft-deleted)
        - Archived document marked with is_deleted=True
        - Archived document excluded from list (include_deleted=False)
        - Archived document can be restored
        - Restored document appears in list again
        """
        # Create a test document
        doc_result, _ = await call_mcp(
            "entity_tool",
    ...
    async def test_list_filtering_operations(self, call_mcp, test_organization, test_project):
        ...
        - Documents can be filtered with limit
...
class TestBulkOperations:
    ...
    async def test_bulk_create_documents(self, call_mcp, test_organization, test_project):
        ...
        Verifies:
        - Bulk data can be prepared with multiple documents
        - Bulk create operation succeeds
        - All documents are created with unique IDs
        - Created documents appear in list operations
        """
        # Prepare bulk data
        bulk_data = {}
        for i in range(3):
...
class TestInvalidOperationsErrorHandling:
    ...
    async def test_invalid_entity_type_error(self, call_mcp):
        ...
        Verifies:
        - Create with invalid entity type fails
        - Error message indicates invalid/unknown entity type
        """
    ...
    async def test_missing_required_field_error(self, call_mcp, test_organization):
        ...
        Verifies:
        - Create without required field handles gracefully
        - Error indicates missing required field (if applicable)
        """
        ...
        if not result.get("success", False):
            # If it fails, should have error about missing required field
