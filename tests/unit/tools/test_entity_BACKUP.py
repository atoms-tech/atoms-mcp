...
# Canonical entity tool tests using parametrized mcp_client fixture
# Tests run against session-scoped FastMCP server via parametrized mcp_client
# which supports unit/integration/e2e variants (currently unit only)
...
async def test_organization(call_mcp):
    ...
    # Skip Pydantic validation for test org creation
    from unittest.mock import patch as mock_patch

    with mock_patch('tools.entity._HAS_SCHEMAS', False):
...
@pytest.mark.dependency(name="TestEntityCreate")
class TestEntityCreate:
    ...
    @pytest.mark.parametrize("entity_type,scenario", [
        ...
        ("test", {"name": "basic"}),  # Entity type is "test", not "test_entity"
        ("property", {"name": "basic"}),
    ])
    async def test_create_entity_parametrized(self, call_mcp, entity_type, scenario, pytestconfig):
        ...
        try:
            # Call the entity tool (call_mcp returns (result, duration_ms))
            ...
            # CallToolResult has .data attribute containing the response dict
            ...
            if success:
                ...
                if entity_type == "organization":
                    ...
                    # Verify auto context resolution worked if applicable
                    if scenario.get("name") == "with_auto_context":
                        assert data.get("organization_id") or "slug" in data
                ...
                test_results.record(entity_type, f"create_{scenario.get('name', 'unknown')}", "FAIL", duration_ms, error_msg)
                pytest.fail(f"Create {entity_type} failed: {error_msg}")
    ...
    async def test_create_document(self, call_mcp, test_organization):
        ...
        assert "data" in project_result and project_result["data"], f"Response missing data: {project_result}"
        assert "id" in project_result["data"], f"Response data missing id: {project_result.get('data')}"
    ...
    async def test_create_requirement(self, call_mcp, test_organization):
        ...
        assert "data" in doc_result and doc_result["data"], f"Response missing data: {doc_result}"
        assert "id" in doc_result["data"], f"Response data missing id: {doc_result.get('data')}"
...
@pytest.mark.dependency(depends=["TestEntityCreate"])
class TestEntityRead:
    ...
    async def test_read_project_with_relations(self, call_mcp, test_organization):
        ...
        assert "data" in create_result and create_result["data"], f"Response missing data: {create_result}"
        assert "id" in create_result["data"], f"Response data missing id: {create_result.get('data')}"
...
@pytest.mark.dependency(depends=["TestEntityCreate", "TestEntityRead"])
...
class TestEntityList:
    ...
    async def test_list_projects_by_organization(self, call_mcp, test_organization):
        ...
        try:
            # First create a project to list
            create_result, _ = await call_mcp(
                "entity_tool",
                {
                    "operation": "create",
                    "entity_type": "project",
                    "data": {
                        "name": f"List Test Project {uuid.uuid4().hex[:8]}",
                        "organization_id": test_organization,
                    },
                },
            )
            ...
            if not create_result.get("success"):
                pytest.skip(f"Could not create project for list test: {create_result.get('error')}")

            # Now list projects by organization
            result, duration = await call_mcp(
                "entity_tool",
                {
                    "operation": "list",
            ...
            if result.get("success"):
                assert "data" in result and result["data"], f"Response missing data: {result}"
                assert isinstance(result["data"], list)
                assert len(result["data"]) > 0, "Should have at least one project after creation"
    ...
    async def test_list_with_pagination(self, call_mcp):
        ...
        try:
            ...
            if page1_result.get("success") and page2_result.get("success"):
                assert "data" in page1_result and page1_result["data"], f"Response missing data: {page1_result}"
                assert "data" in page2_result and page2_result["data"], f"Response missing data: {page2_result}"
...
def generate_report(request):
    ...
    # Use current project directory for report
    report_path = "tests/reports/entity_tool_test_matrix_report.md"
    ...
    print(f"{'='*80}\n")
    # Skip printing the report in CI/automated tests
...
# ===== MERGED FROM test_entity_tool_complete.py - UNIQUE CLASSES =====
# Additional test result tracking classes
...
# ===== B.6 PARAMETRIZATION: Consolidated scenarios to reduce duplication =====
# These parametrized tests implement the patterns from tests/PARAMETRIZATION_ANALYSIS.md
# They consolidate multiple single-purpose tests into table-driven scenarios.

READ_SCENARIOS = [
    {"name": "basic", "include_relations": False, "expect_success": True},
    {"name": "with_relations", "include_relations": True, "expect_success": True},
    {"name": "not_found", "entity_id": "invalid-id", "expect_success": False},
]

SEARCH_SCENARIOS = [
    {"name": "by_term", "search_term": "Test", "expect_success": True},
    {"name": "with_filters", "filters": {"type": "team", "is_deleted": False}, "expect_success": True},
]

PAGINATION_SCENARIOS = [
    {"name": "page_1", "limit": 2, "offset": 0},
    {"name": "page_2", "limit": 2, "offset": 2},
]


class TestEntityReadParam:
    """Parametrized READ operation tests using consolidated scenarios."""

    @pytest.mark.parametrize("scenario", READ_SCENARIOS, ids=[s["name"] for s in READ_SCENARIOS])
    async def test_read_organization_param(self, call_mcp, test_organization, scenario):
        """Read organization by ID with/without relations and not-found case."""
        params = {
            "operation": "read",
            "entity_type": "organization",
            "entity_id": test_organization,
        }
        if "include_relations" in scenario:
            params["include_relations"] = scenario["include_relations"]
        if scenario.get("entity_id") == "invalid-id":
            import uuid as _uuid
            params["entity_id"] = str(_uuid.uuid4())

        result, _ = await call_mcp("entity_tool", params)
        if scenario["expect_success"]:
            assert result.get("success"), f"Expected success, got: {result}"
            assert "data" in result and result["data"], f"Response missing data: {result}"
            assert "id" in result["data"], f"Response data missing id: {result.get('data')}"
            assert result["data"]["id"] == params["entity_id"]
        else:
            assert not result.get("success"), "Expected failure for not-found"


class TestEntitySearchParam:
    """Parametrized SEARCH scenarios for organizations."""

    @pytest.mark.parametrize("scenario", SEARCH_SCENARIOS, ids=[s["name"] for s in SEARCH_SCENARIOS])
    async def test_search_orgs_param(self, call_mcp, scenario):
        """Search organizations by term and with filters."""
        params = {
            "operation": "search",
            "entity_type": "organization",
            "limit": 5,
        }
        if "search_term" in scenario:
            params["search_term"] = scenario["search_term"]
        if "filters" in scenario:
            params["filters"] = scenario["filters"]
...
class TestEntityPaginationParam:
    """Parametrized pagination checks for organization search results."""

    @pytest.mark.unit
    async def test_pagination_disjoint_pages(self, call_mcp):
        """Verify two consecutive pages do not overlap using consolidated params."""
        r1, _ = await call_mcp("entity_tool", {"operation": "search", "entity_type": "organization", "limit": 2, "offset": 0})
        r2, _ = await call_mcp("entity_tool", {"operation": "search", "entity_type": "organization", "limit": 2, "offset": 2})
        if r1.get("success") and r2.get("success"):
            ids1 = {o["id"] for o in r1.get("data", [])}
            ids2 = {o["id"] for o in r2.get("data", [])}
