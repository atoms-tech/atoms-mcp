...
async def test_list_workspaces_default_pagination(authenticated_client):
    ...
    result = await authenticated_client.call_tool("workspace_tool", {"operation": "list_workspaces"})

    assert result["success"], f"Failed to list workspaces: {result.get('error')}"
...
async def test_list_workspaces_custom_limit(authenticated_client):
    ...
    result = await authenticated_client.call_tool("workspace_tool", {"operation": "list_workspaces", "limit": 50})

    assert result["success"], f"Failed to list workspaces with limit=50: {result.get('error')}"

    # Verify the limit was applied
    if "pagination" in result:
        assert result["pagination"].get("limit") == 50, f"Expected limit=50, got {result['pagination'].get('limit')}"
    ...
    assert len(data) <= 50, f"Expected at most 50 items with limit=50, got {len(data)}"
...
async def test_list_workspaces_with_offset(authenticated_client):
    ...
    assert result["success"], f"Failed to list workspaces with limit=25, offset=25: {result.get('error')}"

    # Verify pagination parameters
    if "pagination" in result:
        pagination = result["pagination"]
        assert pagination.get("limit") == 25, f"Expected limit=25, got {pagination.get('limit')}"
        assert pagination.get("offset") == 25, f"Expected offset=25, got {pagination.get('offset')}"
    ...
    # Verify item count
    assert len(data) <= 25, f"Expected at most 25 items with limit=25, got {len(data)}"
...
async def test_list_workspaces_exceeds_max_limit(authenticated_client):
    ...
    result = await authenticated_client.call_tool("workspace_tool", {"operation": "list_workspaces", "limit": 1000})
    ...
    if result["success"]:
        ...
        if "pagination" in result:
            ...
            assert actual_limit <= 1000, f"Expected limit <= 1000, got {actual_limit}"
...
async def test_list_workspaces_with_data_check(authenticated_client):
    ...
    assert result["success"], f"Failed to list workspaces for data check: {result.get('error')}"
    ...
    if data and len(data) > 0:
        ...
        assert has_core, f"Expected core fields {core_fields} in workspace items, got keys: {list(first_item.keys())}"
...
async def test_list_workspaces_consistency_check(authenticated_client):
    ...
    for i in range(2):
        ...
        assert result["success"], f"Failed to list workspaces on iteration {i + 1}: {result.get('error')}"
    ...
    if len(results) == 2:
        ...
        assert "data" in results[0] and "data" in results[1], "Both responses should have 'data' key"
...
async def test_get_context_basic(authenticated_client):
    ...
    result = await authenticated_client.call_tool("workspace_tool", {"operation": "get_context"})

    assert result["success"], f"Failed to get context: {result.get('error')}"
    ...
    assert has_context_info, f"Expected context information in result, got keys: {list(result.keys())}"
...
async def test_get_context_structure(authenticated_client):
    ...
    assert result["success"], f"Failed to get context for structure check: {result.get('error')}"
...
async def test_set_context_organization(authenticated_client):
    ...
    list_result = await authenticated_client.call_tool("workspace_tool", {"operation": "list_workspaces"})
    ...
    assert result["success"], f"Failed to set organization context for ID {org_id}: {result.get('error')}"
    ...
    if "data" in result:
        ...
        if isinstance(context_info, dict):
            ...
            if "id" in context_info or "organization_id" in context_info:
                ...
                assert actual_id == org_id, f"Expected organization ID {org_id}, got {actual_id}"
...
async def test_set_context_project(authenticated_client):
    ...
    # Set project context
    context_params = {"operation": "set_context", "context_type": "project", "entity_id": project_id}
    ...
    assert result["success"], f"Failed to set project context for ID {project_id}: {result.get('error')}"
    ...
    if "data" in result:
        ...
        if isinstance(context_info, dict):
            ...
            if "id" in context_info or "project_id" in context_info:
                ...
                assert actual_id == project_id, f"Expected project ID {project_id}, got {actual_id}"
...
async def test_set_context_document(authenticated_client):
    ...
    context_params = {"operation": "set_context", "context_type": "document", "entity_id": document_id}
    ...
    assert result["success"], f"Failed to set document context for ID {document_id}: {result.get('error')}"
    ...
    if "data" in result:
        ...
        if isinstance(context_info, dict):
            ...
            if "id" in context_info or "document_id" in context_info:
                ...
                assert actual_id == document_id, f"Expected document ID {document_id}, got {actual_id}"
...
async def test_get_defaults_basic(authenticated_client):
    ...
    result = await authenticated_client.call_tool("workspace_tool", {"operation": "get_defaults"})

    assert result["success"], f"Failed to get defaults: {result.get('error')}"
    ...
    if defaults_data and isinstance(defaults_data, dict):
        ...
        assert isinstance(defaults_data, dict), f"Expected defaults_data to be a dict, got {type(defaults_data)}"
...
async def test_get_defaults_data_types(authenticated_client):
    ...
    assert result["success"], f"Failed to get defaults for data type check: {result.get('error')}"

    # get_defaults might return data in different ways
    # Check for default settings in result
    default_fields = ["organization_id", "project_id", "document_id", "defaults", "settings", "preferences", "data"]
...
async def test_list_workspaces_performance(authenticated_client):
    ...
    assert result["success"], f"Failed to list workspaces in performance test: {result.get('error')}"
    assert duration < 2.0, (
...
async def test_get_context_performance(authenticated_client):
    ...
    assert result["success"], f"Failed to get context in performance test: {result.get('error')}"
    assert duration < 1.0, (
