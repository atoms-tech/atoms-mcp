...
async def test_setup_project_minimal_params(authenticated_client):
    ...
    org_result = await authenticated_client.call_tool(
        "entity_tool",
        {
            ...
            "data": {"name": f"Test Org {timestamp}", "slug": f"test-org-{timestamp}"},
        },
    )
    ...
    result = await authenticated_client.call_tool(
        "workflow_tool",
        {"workflow": "setup_project", "parameters": {"name": "Minimal Test Project", "organization_id": org_id}},
    )
    duration_ms = (time.time() - start_time) * 1000

    # Assertions
    assert result.get("success") is not False, f"Workflow failed: {result.get('error')}"
    assert result.get("project_id") or (
        result.get("data", {}).get("project_id") if isinstance(result.get("data"), dict) else None
    ...
    assert duration_ms < 5000, f"Workflow took {duration_ms:.0f}ms, should be < 5000ms with FastHTTPClient"
...
async def test_setup_project_with_description(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "workflow_tool",
        {
            ...
            "parameters": {"name": "Described Project", "organization_id": org_id, "description": description},
        },
    )
    ...
    project_result = await authenticated_client.call_tool(
        "entity_tool", {"operation": "read", "entity_type": "project", "entity_id": project_id}
    )
...
async def test_setup_project_add_creator_as_admin_true(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        ...
        {
            "workflow": "setup_project",
            "parameters": {"name": "Admin Project", "organization_id": org_id, "add_creator_as_admin": True},
        },
    )
    ...
    assert any(step.get("step") == "add_creator_member" and step.get("status") == "success" for step in results), (
...
async def test_setup_project_with_initial_documents(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        ...
        {
            "workflow": "setup_project",
            "parameters": {"name": "Project with 3 Docs", "organization_id": org_id, "initial_documents": doc_names},
        },
    )
    ...
    results = result.get("results", [])
    doc_steps = [step for step in results if step.get("step", "").startswith("create_document_")]
    assert len(doc_steps) == 3, f"Should create 3 documents, found {len(doc_steps)} document creation steps"
...
async def test_setup_project_invalid_organization_id(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "workflow_tool",
        {"workflow": "setup_project", "parameters": {"name": "Invalid Org Project", "organization_id": invalid_org_id}},
    )

    assert result.get("success") is False, "Should fail with invalid org ID, but got success"
...
async def setup_document(authenticated_client) -> tuple[str, str, str]:
    ...
    project_result = await authenticated_client.call_tool(
        "entity_tool",
        {
            "operation": "create",
            "entity_type": "project",
            "data": {"name": f"Test Project {timestamp}", "organization_id": org_id},
        },
    )
    ...
    doc_result = await authenticated_client.call_tool(
        "entity_tool",
        {
            ...
            "data": {"name": f"Test Document {timestamp}", "project_id": project_id},
        },
    )
...
async def test_import_requirements_small_batch(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "workflow_tool",
        {"workflow": "import_requirements", "parameters": {"document_id": doc_id, "requirements": requirements}},
    )
    duration_ms = (time.time() - start_time) * 1000
    ...
    assert duration_ms < 5000, f"Import took {duration_ms:.0f}ms, should be < 5000ms with FastHTTPClient"
...
async def test_import_requirements_with_full_metadata(authenticated_client):
    ...
    for step in results:
        if step.get("status") == "success" and "result" in step:
            req = step["result"]
            assert req.get("status") in ["active", "approved"], f"Invalid status: {req.get('status')}"
            assert req.get("priority") in ["high", "medium"], f"Invalid priority: {req.get('priority')}"
...
async def test_import_requirements_invalid_document_id(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "workflow_tool",
        {
            ...
            "parameters": {"document_id": invalid_doc_id, "requirements": requirements},
        },
    )

    assert result.get("success") is False, "Should fail with invalid document ID, but got success"
    assert "error" in result, f"Expected 'error' in result, got keys: {list(result.keys())}"
...
async def test_import_requirements_empty_list(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "workflow_tool", {"workflow": "import_requirements", "parameters": {"document_id": doc_id, "requirements": []}}
    )

    assert result.get("success") is False, "Should fail with empty requirements list, but got success"
    error = result.get("error", "").lower()
    assert "empty" in error or "non-empty" in error, f"Expected 'empty' error message, got: {result.get('error')}"
...
async def setup_project_with_requirements(
    ...
    for i in range(requirement_count):
        ...
        req_result = await authenticated_client.call_tool(
            "entity_tool", {"operation": "create", "entity_type": "requirement", "data": req_data}
        )
...
async def test_setup_test_matrix_default_name(authenticated_client):
    ...
    assert result.get("success") is not False, f"Workflow failed: {result.get('error')}"
    assert result.get("matrix_view_id"), "No matrix_view_id in response"
...
async def test_setup_test_matrix_custom_name(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        ...
        {
            "workflow": "setup_test_matrix",
            "parameters": {"project_id": project_id, "matrix_name": "Custom Test Coverage Matrix"},
        },
    )
...
async def test_setup_test_matrix_empty_project(authenticated_client):
    ...
    project_result = await authenticated_client.call_tool(
        "entity_tool",
        {
            "operation": "create",
            "entity_type": "project",
            "data": {"name": f"Empty Project {timestamp}", "organization_id": org_id},
        },
    )
    ...
    result = await authenticated_client.call_tool(
        "workflow_tool", {"workflow": "setup_test_matrix", "parameters": {"project_id": project_id}}
    )
...
async def create_test_entities(authenticated_client, entity_type: str, count: int) -> list[str]:
...
async def test_bulk_status_update_small_batch(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "workflow_tool",
        {
            ...
            "parameters": {"entity_type": "requirement", "entity_ids": entity_ids, "new_status": "active"},
        },
    )
    duration_ms = (time.time() - start_time) * 1000
    ...
    assert duration_ms < 5000, f"Bulk update took {duration_ms:.0f}ms, should be < 5000ms with FastHTTPClient"
...
async def test_bulk_status_update_different_statuses(authenticated_client):
    ...
    for status in ["active", "approved", "rejected"]:
        result = await authenticated_client.call_tool(
            ...
            {
                "workflow": "bulk_status_update",
                "parameters": {"entity_type": "requirement", "entity_ids": entity_ids, "new_status": status},
            },
        )

        assert result.get("success") is not False, f"Workflow failed for status '{status}': {result.get('error')}"
...
async def test_bulk_status_update_partial_failure(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        ...
        {
            "workflow": "bulk_status_update",
            "parameters": {"entity_type": "requirement", "entity_ids": all_ids, "new_status": "active"},
        },
    )

    # Should still be successful but with partial results
    assert result.get("success") is not False, f"Workflow should handle partial failures, got: {result.get('error')}"
...
async def test_organization_onboarding_minimal(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "workflow_tool",
        {"workflow": "organization_onboarding", "parameters": {"name": f"Minimal Test Organization {timestamp}"}},
    )

    assert result.get("success") is not False, f"Workflow failed: {result.get('error')}"
    assert result.get("organization_id"), "No organization_id in response"
...
async def test_organization_onboarding_with_slug(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        "workflow_tool",
        {
            ...
            "parameters": {"name": "Organization with Custom Slug", "slug": custom_slug},
        },
    )
...
async def test_organization_onboarding_with_starter_project(authenticated_client):
    ...
    result = await authenticated_client.call_tool(
        ...
        {
            "workflow": "organization_onboarding",
            "parameters": {"name": f"Org with Starter Project {timestamp}", "create_starter_project": True},
        },
    )
    ...
    results = result.get("results", [])
    project_steps = [step for step in results if step.get("step") == "create_starter_project"]
    assert len(project_steps) == 1, f"Expected 1 starter project creation step, got {len(project_steps)}"
...
async def test_organization_onboarding_all_parameters(authenticated_client):
    ...
    assert duration_ms < 8000, f"Complex workflow took {duration_ms:.0f}ms, should be < 8000ms with FastHTTPClient"
    ...
    for step in results:
        if step.get("step") == "create_organization" and "result" in step:
            org = step["result"]
            assert org.get("name") == "Complete Organization", f"Name mismatch: {org.get('name')}"
            assert org.get("slug") == custom_slug, f"Slug mismatch: {org.get('slug')}"
            assert org.get("type") == "enterprise", f"Type mismatch: {org.get('type')}"
...
async def test_organization_onboarding_missing_name(authenticated_client):
    ...
    assert result.get("success") is False, "Should fail with missing name, but got success"
    assert "error" in result, f"Expected 'error' in result, got keys: {list(result.keys())}"
    error = result.get("error", "").lower()
    assert "name" in error, f"Expected 'name' in error message, got: {result.get('error')}"
...
async def test_workflow_performance(authenticated_client):
    ...
    org_result = await authenticated_client.call_tool(
        "entity_tool",
        {
            "operation": "create",
            "entity_type": "organization",
            "data": {"name": f"Perf Test Org {timestamp}", "slug": f"perf-test-org-{timestamp}"},
        },
    )
    ...
    result = await authenticated_client.call_tool(
        ...
        {
            "workflow": "setup_project",
            "parameters": {"name": f"Performance Test Project {timestamp}", "organization_id": org_id},
        },
    )
