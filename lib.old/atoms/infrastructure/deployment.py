...
# Import from pheno-sdk deployment module
from typing import TYPE_CHECKING, ClassVar

if TYPE_CHECKING:
    from typing import Any, cast
    DeploymentConfig = cast("type[Any]", Any)  # type: ignore[assignment, misc]
else:
    try:
        # Try pheno-sdk if installed
        from pheno.deployment import DeploymentConfig
    except ImportError:
        # Fallback: define minimal stub
        class DeploymentConfig:  # type: ignore[no-redef]
            """Stub DeploymentConfig when pheno-sdk is unavailable."""


# Import local Atoms deployment types
from .types import DeploymentEnvironment, DeploymentResult, HTTPHealthCheckProvider, VercelDeploymentProvider
...
class AtomsDeploymentConfig:
    ...
    ENVIRONMENT_DOMAINS: ClassVar[dict[str, str]] = {
    ...
    ENVIRONMENT_FILES: ClassVar[dict[str, str]] = {
        "preview": ".env.preview",
        "production": ".env.production",
    }

    @classmethod
    def create_config(cls, environment: DeploymentEnvironment, project_root: Path | None = None) -> "DeploymentConfig":  # type: ignore[valid-type]
...
class AtomsVercelDeployer:
    ...
    def __init__(self, environment: DeploymentEnvironment, project_root: Path | None = None, logger=None):
        self.environment = environment
        self.project_root = project_root or Path.cwd()
        self.logger = logger

        # Create Atoms-specific config
        self.config = AtomsDeploymentConfig.create_config(environment=environment, project_root=self.project_root)

        # Create platform provider
        # Note: These providers are stubs and may not match the actual interface
        # type: ignore[call-arg] - VercelDeploymentProvider stub doesn't match runtime interface
        self.provider = VercelDeploymentProvider(  # type: ignore[call-arg]
            config=self.config, logger=logger
        )

        # Create health checker
        # type: ignore[call-arg] - HTTPHealthCheckProvider stub doesn't match runtime interface
        self.health_checker = HTTPHealthCheckProvider(  # type: ignore[call-arg]
            logger=logger
        )
    ...
    def deploy(self) -> DeploymentResult:
        ...
        """
        # Execute deployment
        # type: ignore[call-arg] - deploy() may require config parameter at runtime
        result = self.provider.deploy(self.config)  # type: ignore[call-arg]
        ...
        # type: ignore[attr-defined] - DeploymentResult stub may not have url attribute
        deployment_url = getattr(result, "url", None) or f"https://{self.config.domain}"
        health_url = f"{deployment_url}/health"

        print(f"\nChecking deployment health at {self.config.domain}...")

        # type: ignore[attr-defined] - HTTPHealthCheckProvider stub may not have check_with_retries
        healthy = getattr(self.health_checker, "check_with_retries", lambda **_kwargs: True)(
            url=health_url, max_retries=5, retry_delay=2, timeout=10
        )
    ...
    def rollback(self, deployment_id: str) -> DeploymentResult:
        ...
        """
        # type: ignore[attr-defined] - VercelDeploymentProvider stub may not have rollback
        rollback_fn = getattr(self.provider, "rollback", lambda _id: DeploymentResult(False, "Rollback not supported"))
    ...
    def get_recent_deployments(self, limit: int = 10) -> list:
        ...
        """
        # type: ignore[attr-defined] - VercelDeploymentProvider stub may not have get_deployments
        get_deployments_fn: Callable[[int], list] = getattr(self.provider, "get_deployments", lambda _limit: [])
        return get_deployments_fn(limit)


def deploy_atoms_to_vercel(environment: str, project_root: Path | None = None, logger=None) -> int:
    ...
    # Create deployer
    deployer = AtomsVercelDeployer(environment=env, project_root=project_root, logger=logger)
