...
class AtomsServerManager:
    ...
    def __init__(self, port: int | None = None, verbose: bool = False, no_tunnel: bool = False, logger=None):
        ...
        if port is None and PHENO_SDK_AVAILABLE:
            try:
                ...
                # Run cleanup with safe wrapper
                try:
                    cleanup_stats = run_async_safely(cleanup_before_startup("atoms-mcp-server", cleanup_config))
                    if logger:
    ...
    def start(self) -> int:
        ...
        cmd = [sys.executable, str(Path(__file__).parent.parent.parent / "start_server.py")]
...
def start_atoms_server(port: int | None = None, verbose: bool = False, no_tunnel: bool = False, logger=None) -> int:
    ...
    Returns:
        Exit code (0 for success)
    """
    manager = AtomsServerManager(port=port, verbose=verbose, no_tunnel=no_tunnel, logger=logger)
