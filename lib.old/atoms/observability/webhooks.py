...
class WebhookClient:
    ...
    async def send(self, payload: WebhookPayload) -> bool:
        ...
        if not self.config.should_send_event(payload.event_type):
            logger.debug(f"Skipping webhook {self.config.name} for event {payload.event_type.value}")
            return False
...
class WebhookManager:
    ...
    async def send_notification(self, payload: WebhookPayload) -> dict[str, bool]:
        ...
        tasks = [webhook.send(payload) for webhook in self.webhooks]
        ...
        for webhook, result in zip(self.webhooks, results, strict=False):
            if isinstance(result, Exception):
                logger.error(
                    ...
                    extra_fields={"webhook_name": webhook.config.name, "error": str(result)},
                )
                results_dict[webhook.config.name] = False
            else:
                # Type narrowing: result is bool here (from webhook.send())
    ...
    async def send_deployment_started(
        self, deployment_id: str, environment: str, metadata: dict[str, Any] | None = None
    ) -> dict[str, bool]:
    ...
    async def send_deployment_completed(
        self, deployment_id: str, environment: str, duration_seconds: float, metadata: dict[str, Any] | None = None
    ) -> dict[str, bool]:
        ...
        payload = WebhookPayload(
            ...
            environment=environment,
            metadata={"deployment_id": deployment_id, "duration_seconds": duration_seconds, **(metadata or {})},
        )
        return await self.send_notification(payload)

    async def send_deployment_failed(
        self, deployment_id: str, environment: str, error: str, metadata: dict[str, Any] | None = None
    ) -> dict[str, bool]:
        ...
        payload = WebhookPayload(
            ...
            environment=environment,
            metadata={"deployment_id": deployment_id, "error": error, **(metadata or {})},
        )
        return await self.send_notification(payload)

    async def send_error_alert(
        self, error_type: str, error_message: str, source: str, metadata: dict[str, Any] | None = None
    ) -> dict[str, bool]:
        ...
        payload = WebhookPayload(
            ...
            metadata={"error_type": error_type, "source": source, **(metadata or {})},
        )
        return await self.send_notification(payload)

    async def send_warning_alert(
        self, warning_type: str, warning_message: str, source: str, metadata: dict[str, Any] | None = None
    ) -> dict[str, bool]:
        ...
        payload = WebhookPayload(
            ...
            metadata={"warning_type": warning_type, "source": source, **(metadata or {})},
        )
        return await self.send_notification(payload)

    async def send_health_degraded(
        self, component_name: str, health_status: HealthStatus, message: str, metadata: dict[str, Any] | None = None
    ) -> dict[str, bool]:
        """Send health degradation alert."""
        severity = AlertSeverity.CRITICAL if health_status == HealthStatus.UNHEALTHY else AlertSeverity.WARNING
        ...
        payload = WebhookPayload(
            ...
            message=message,
            metadata={"component_name": component_name, "health_status": health_status.value, **(metadata or {})},
        )
        return await self.send_notification(payload)

    async def send_health_recovered(
        self, component_name: str, message: str, metadata: dict[str, Any] | None = None
    ) -> dict[str, bool]:
        ...
        payload = WebhookPayload(
            ...
            message=message,
            metadata={"component_name": component_name, **(metadata or {})},
        )
        return await self.send_notification(payload)

    async def send_performance_alert(
        self, operation: str, duration_ms: float, threshold_ms: float, metadata: dict[str, Any] | None = None
    ) -> dict[str, bool]:
...
def configure_vercel_webhook(webhook_url: str, event_types: list[WebhookEventType] | None = None) -> None:
    """
