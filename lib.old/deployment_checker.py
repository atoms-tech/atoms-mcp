...
class DeploymentChecker:
    ...
    def check_directory_exists(self, path: str, _name: str) -> tuple[bool, str]:
        """Check if a directory exists."""
        dir_path = self.project_root / path
        if dir_path.exists() and dir_path.is_dir():
            # Count items
            items = [
                item for item in dir_path.iterdir() if not item.name.startswith(".") and item.name != "__pycache__"
            ]
            return True, f"{path}/ exists with {len(items)} items"
        return False, f"{path}/ not found"

    def check_file_exists(self, path: str, _name: str) -> tuple[bool, str]:
    ...
    def check_file_executable(self, path: str, _name: str) -> tuple[bool, str]:
    ...
    def check_file_not_contains(self, path: str, pattern: str, _name: str) -> tuple[bool, str]:
    ...
    def check_file_contains(self, path: str, pattern: str, _name: str) -> tuple[bool, str]:
    ...
    def check_no_uncommitted_changes(self) -> tuple[bool, str]:
        ...
        try:
            ...
            subprocess.run(["git", "rev-parse", "--git-dir"], cwd=self.project_root, capture_output=True, check=True)

            # Check for changes
            result = subprocess.run(["git", "diff", "--quiet"], check=False, cwd=self.project_root, capture_output=True)

            result2 = subprocess.run(
                ["git", "diff", "--cached", "--quiet"], check=False, cwd=self.project_root, capture_output=True
            )
...
def create_vercel_deployment_checks(project_root: Path | None = None) -> list[DeploymentCheck]:
