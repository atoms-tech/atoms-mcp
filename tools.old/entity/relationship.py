...
class RelationshipManager(ToolBase):
    ...
    async def link_entities(
            ...
            if source.get("type") == "organization":
                ...
                if not await is_organization_owner_or_admin(org_id, user_id, db_adapter):
                    raise PermissionDeniedError(
                        "organization_members", "INSERT", "Only organization owners and admins can add members"
                    )
                ...
                if not await is_project_owner_or_admin(project_id, user_id, db_adapter):
                    raise PermissionDeniedError(
                        "project_members", "INSERT", "Only project owners and admins can add members"
                    )
        ...
        # RLS validation for assignment relationships
            ...
            if source_type in ["requirement", "test", "document"]:
                ...
                source_entity = await self._db_get_single(
                    self._resolve_entity_table(source_type), filters={"id": source["id"]}
                )
                if source_entity:
        ...
        # RLS validation for trace links
            ...
            if source_type in ["requirement", "test", "document"]:
                ...
                if source_entity:
                    ...
                    if project_id and not await user_can_access_project(project_id, user_id, db_adapter):
                        raise PermissionDeniedError(
                            "trace_links", "INSERT", "User does not have access to source entity"
                        )

            # Validate target access
            if target_type in ["requirement", "test", "document"]:
                target_entity = await self._db_get_single(
                    self._resolve_entity_table(target_type), filters={"id": target["id"]}
                )
                if target_entity:
                    project_id = target_entity.get("project_id")
                    if project_id and not await user_can_access_project(project_id, user_id, db_adapter):
                        raise PermissionDeniedError(
                            "trace_links", "INSERT", "User does not have access to target entity"
                        )
        ...
        if relationship_type == RelationshipType.MEMBER:
            if source.get("type") in ["organization", "project"]:
                ...
                link_data = {table_config["source_field"]: source["id"], table_config["target_field"]: target["id"]}
            ...
            table = config["table"]
            link_data = {config["source_field"]: source["id"], config["target_field"]: target["id"]}
        ...
        # Create the link
        return await self._db_insert(table, link_data, returning="*")

    async def unlink_entities(
        self, relationship_type: str, source: dict[str, str], target: dict[str, str], soft_delete: bool = True
    ) -> bool:
        ...
        if relationship_type == RelationshipType.MEMBER:
            if source.get("type") == "organization":
                ...
                if not await is_organization_owner_or_admin(org_id, user_id, db_adapter):
                    raise PermissionDeniedError(
                        "organization_members", "DELETE", "Only organization owners and admins can remove members"
                    )
            elif source.get("type") == "project":
                # Validate user can manage project members
                project_id = source["id"]
                if not await is_project_owner_or_admin(project_id, user_id, db_adapter):
                    raise PermissionDeniedError(
                        "project_members", "DELETE", "Only project owners and admins can remove members"
                    )
        ...
        if relationship_type == RelationshipType.MEMBER:
            if source.get("type") in ["organization", "project"]:
                ...
                filters = {table_config["source_field"]: source["id"], table_config["target_field"]: target["id"]}
            else:
                raise ValueError(f"Invalid source type for member relationship: {source.get('type')}")
        else:
            table = config["table"]
            filters = {config["source_field"]: source["id"], config["target_field"]: target["id"]}

        if soft_delete and "is_deleted" in config.get("defaults", {}):
            # Soft delete
            update_data = {"is_deleted": True, "deleted_at": datetime.now(UTC).isoformat()}
    ...
    async def list_relationships(
        ...
        relationships = await self._db_query(
            table, select="*", filters=query_filters, limit=limit, offset=offset, order_by="created_at:desc"
        )
                ...
                if relationship_type == RelationshipType.MEMBER:
                    ...
                    if source and source.get("type") == "organization":
                        org_member = await db_adapter.get_single(
                            ...
                            filters={
                                "organization_id": rel.get("organization_id"),
                                "user_id": user_id,
                                "is_deleted": False,
                            },
                        )
        ...
        if relationship_type == RelationshipType.MEMBER and relationships:
            ...
            if user_ids:
                # Fetch profiles
                profiles = await self._db_query(Tables.PROFILES, filters={"id": {"in": user_ids}})
    ...
    async def check_relationship(
        self, relationship_type: str, source: dict[str, str], target: dict[str, str]
    ) -> dict[str, Any] | None:
        """Check if a relationship exists."""
        relationships = await self.list_relationships(relationship_type, source, target)
        return relationships[0] if relationships else None

    async def update_relationship(
        self, relationship_type: str, source: dict[str, str], target: dict[str, str], metadata: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if relationship_type == RelationshipType.MEMBER:
            if source.get("type") == "organization":
                ...
                if not await is_organization_owner_or_admin(org_id, user_id, db_adapter):
                    raise PermissionDeniedError(
                        "organization_members", "UPDATE", "Only organization owners and admins can update member roles"
                    )
            elif source.get("type") == "project":
                # Validate user can manage project members
                project_id = source["id"]
                if not await is_project_owner_or_admin(project_id, user_id, db_adapter):
                    raise PermissionDeniedError(
                        "project_members", "UPDATE", "Only project owners and admins can update member roles"
                    )
        ...
        return await self._db_update(table, update_data, filters, returning="*")
...
async def relationship_operation(
    ...
    """
    # Inner helper functions for error handling
    def raise_link_target_error() -> None:
        """Raise error when target is missing for link operation."""
        raise ValueError("target is required for link operation")

    def raise_unlink_target_error() -> None:
        """Raise error when target is missing for unlink operation."""
        raise ValueError("target is required for unlink operation")

    def raise_check_target_error() -> None:
        """Raise error when target is missing for check operation."""
        raise ValueError("target is required for check operation")

    def raise_update_params_error() -> None:
        """Raise error when target or metadata is missing for update operation."""
        raise ValueError("target and metadata are required for update operation")

    def raise_unknown_operation_error(op: str) -> None:
    ...
    try:
        ...
        if operation == "link":
            if not target:
                raise_link_target_error()

            assert target is not None  # For mypy - target is guaranteed to be not None after the check above

            result = await _relationship_manager.link_entities(
                relationship_type, source, target, metadata or {}, source_context
            )
            return _relationship_manager._format_result(result, format_type)

        if operation == "unlink":
            if not target:
                raise_unlink_target_error()

            assert target is not None  # For mypy

            success = await _relationship_manager.unlink_entities(relationship_type, source, target, soft_delete)
            return {"success": success, "relationship_type": relationship_type, "source": source, "target": target}
        ...
        if operation == "update":
            if not target or not metadata:
                raise_update_params_error()

            assert target is not None  # For mypy
            assert metadata is not None  # For mypy

            result = await _relationship_manager.update_relationship(relationship_type, source, target, metadata)
            return _relationship_manager._format_result(result, format_type)

        raise_unknown_operation_error(operation)
