...
try:
    from ..entity_resolver import EntityResolver  # type: ignore[no-redef]
except ImportError:
    from tools.entity_resolver import EntityResolver  # type: ignore[no-redef]

try:
    from ..workspace import _workspace_manager  # type: ignore[no-redef]
except ImportError:
    from tools.workspace import _workspace_manager  # type: ignore[no-redef]

try:
    from config.vector import get_progressive_embedding_service
except ImportError:  # pragma: no cover - optional dependency
    # Import failed, define as None - using ignore to silence redefinition if already imported
    get_progressive_embedding_service: Callable[..., Any] | None = None  # type: ignore[no-redef]
...
class EntityManager(ToolBase):
    ...
    def _apply_defaults(self, entity_type: str, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if Fields.CREATED_BY not in result:
            if not user_id:
                raise ValueError(
                    f"Cannot create {entity_type}: user_id not available in context. Check authentication."
                )
        ...
        if entity_type.lower() == EntityType.REQUIREMENT.value and Fields.EXTERNAL_ID not in result:
            ...
            result[Fields.EXTERNAL_ID] = f"REQ-{int(datetime.now(UTC).timestamp())}"
        ...
        if (
            entity_type.lower() in [EntityType.ORGANIZATION.value, EntityType.DOCUMENT.value]
            and not result.get(Fields.SLUG)
            and result.get(Fields.NAME)
        ):
            result[Fields.SLUG] = _slugify(result[Fields.NAME])
    ...
    async def _resolve_smart_defaults(self, entity_type: str, data: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.debug("Resolving smart defaults for %s", entity_type)
    ...
    async def create_entity(
        self, entity_type: str, data: dict[str, Any], include_relations: bool = False
    ) -> dict[str, Any]:
        """Create a new entity."""
        ...
        try:
            ...
            raise ValueError(f"Trigger validation failed: {e}") from e
        ...
        if policy:
            try:
                ...
                raise ValueError(f"Permission denied: {e.reason}") from e
        ...
        for effect_table, effect_data in side_effects:
            try:
                ...
                logger.warning("Side effect insert failed for %s: %s", effect_table, e)

        if include_relations:
            logger.debug(
                "include_relations requested during create; fetch relationships via read() to avoid client issues"
            )
        ...
        if entity_type in ["organization", "project", "document", "requirement"]:
            task = asyncio.create_task(self._generate_embedding_async(entity_type, result))
            self._background_tasks.add(task)
            task.add_done_callback(lambda t: self._background_tasks.discard(t))  # type: ignore[arg-type]
    ...
    async def _generate_embedding_async(self, entity_type: str, entity_data: dict[str, Any]):
        ...
        """
        if get_progressive_embedding_service is None:
            logger.warning("Progressive embedding service unavailable; skipping embedding for %s", entity_type)
            return
        ...
        try:
            ...
            # Generate embedding
            await progressive_service.generate_embedding_on_demand(table_name, entity_data["id"], entity_data)

        except Exception as e:
            logger.warning("Failed to generate embedding for %s %s: %s", entity_type, entity_data.get("id"), e)

    async def read_entity(
        self, entity_type: str, entity_id: str, include_relations: bool = False
    ) -> dict[str, Any] | None:
        """Read an entity by ID."""
        table = self._resolve_entity_table(entity_type)

        result = await self._db_get_single(table, filters={"id": entity_id})
    ...
    async def update_entity(
        self, entity_type: str, entity_id: str, data: dict[str, Any], include_relations: bool = False
    ) -> dict[str, Any]:
        """Update an entity."""
        ...
        existing_record = await self._db_get_single(table, filters={"id": entity_id})
        ...
        # Update entity
        result = await self._db_update(table, update_data, filters={"id": entity_id}, returning="*")
        ...
        for effect_table, effect_data in side_effects:
            try:
                ...
                # Log side effect failures but don't fail the main operation
                logger.warning("Side effect from update failed for %s: %s", effect_table, e)
    ...
    async def delete_entity(self, entity_type: str, entity_id: str, soft_delete: bool = True) -> bool:
            ...
            if not user_id:
                ...
                try:
                    result = (
                        ...
                        .select(f"{Fields.CREATED_BY}, {Fields.UPDATED_BY}")
                        .eq(Fields.ID, entity_id)
                        .execute()
                    )
    ...
    async def search_entities(
        ...
        return await self._db_query(table, filters=query_filters, limit=limit, offset=offset, order_by=order_by)

    async def list_entities(
        self, entity_type: str, parent_type: str | None = None, parent_id: str | None = None, limit: int | None = None
    ) -> list[dict[str, Any]]:
        ...
        return await self.search_entities(entity_type, filters=filters, limit=limit)

    async def _include_relationships(self, entity_type: str, entity: dict[str, Any]) -> dict[str, Any]:
        ...
        if entity_type.lower() == EntityType.ORGANIZATION.value:
            ...
            member_count = await self._db_count(
                Tables.ORGANIZATION_MEMBERS, filters={Fields.ORGANIZATION_ID: entity_id, Fields.STATUS: "active"}
            )
            ...
            doc_count = await self._db_count(
                Tables.DOCUMENTS, filters={Fields.PROJECT_ID: entity_id, Fields.IS_DELETED: False}
            )
            ...
            req_count = await self._db_count(
                Tables.REQUIREMENTS, filters={Fields.DOCUMENT_ID: entity_id, Fields.IS_DELETED: False}
            )
            result["requirement_count"] = req_count

            blocks = await self._db_query(
                Tables.BLOCKS, filters={Fields.DOCUMENT_ID: entity_id, Fields.IS_DELETED: False}, order_by="position"
            )
...
def _require_payload(value: dict[str, Any] | None, message: str) -> dict[str, Any]:
    if not value:
        _raise_value_error(message)
    return value


def _require_identifier(value: str | None, message: str) -> str:
    if not value:
        _raise_value_error(message)
    return value
...
def _finalize_operation(
    ...
    return entity_manager._add_timing_metrics(result, timings)
...
async def _handle_create_operation(
    ...
    payload = _require_payload(data, "data is required for create operation")
    t_op_start = time.time()
    if batch:
        tasks = [entity_manager.create_entity(entity_type, item, include_relations) for item in batch]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        final_results: list[dict[str, object]] = []
        for index, result in enumerate(results):
            if isinstance(result, Exception):
                final_results.append({"error": str(result), "index": index})
            elif isinstance(result, dict):
                final_results.append(result)

        timings["batch_create"] = time.time() - t_op_start
        formatted = entity_manager._format_result(final_results, format_type)
        return _finalize_operation(entity_manager, timings, start_total, formatted)

    result = await entity_manager.create_entity(entity_type, payload, include_relations)
    timings["create"] = time.time() - t_op_start
    formatted = entity_manager._format_result(result, format_type)
    return _finalize_operation(entity_manager, timings, start_total, formatted)
...
async def _handle_read_operation(
    ...
    identifier = _require_identifier(entity_id, "entity_id is required for read operation")
    t_op_start = time.time()
    read_result: dict[str, Any] | None = await entity_manager.read_entity(entity_type, identifier, include_relations)
    timings["read"] = time.time() - t_op_start

    if read_result is None:
        resolver = EntityResolver(entity_manager._get_adapters()["database"])
        ...
        error_response = {
            "success": False,
            "error": f"{entity_type} with ID '{identifier}' not found",
        }
    ...
    formatted = entity_manager._format_result(read_result, format_type)
    return _finalize_operation(entity_manager, timings, start_total, formatted)
...
async def _handle_update_operation(
    ...
    identifier = _require_identifier(entity_id, "entity_id and data are required for update operation")
    payload = _require_payload(data, "entity_id and data are required for update operation")
    t_op_start = time.time()
    result = await entity_manager.update_entity(entity_type, identifier, payload, include_relations)
...
async def _handle_delete_operation(
    ...
    identifier = _require_identifier(entity_id, "entity_id is required for delete operation")
    t_op_start = time.time()
    success = await entity_manager.delete_entity(entity_type, identifier, soft_delete)
    ...
    return _finalize_operation(entity_manager, timings, start_total, result)
...
async def _handle_search_operation(
    ...
    t_op_start = time.time()
    search_results: list[dict[str, Any]] = await entity_manager.search_entities(
        entity_type, filters, search_term, limit, offset, order_by
    )
    timings["search"] = time.time() - t_op_start
    formatted = entity_manager._format_result(search_results, format_type)
    return _finalize_operation(entity_manager, timings, start_total, formatted)
...
async def _handle_list_operation(
    ...
    t_op_start = time.time()
    list_results: list[dict[str, Any]] = await entity_manager.list_entities(entity_type, parent_type, parent_id, limit)
    timings["list"] = time.time() - t_op_start
    formatted = entity_manager._format_result(list_results, format_type)
    return _finalize_operation(entity_manager, timings, start_total, formatted)
...
async def entity_operation(
    ...
    try:
        ...
        if entity_id and not _entity_manager._is_uuid_format(entity_id):
            ...
            resolution = await resolver.resolve_entity_id(entity_type, entity_id, filters=filters, threshold=70)
            ...
            if "note" in resolution:
                timings["fuzzy_match_note"] = resolution["note"]  # type: ignore[assignment]

        operation_handlers = {
            "create": partial(
                _handle_create_operation,
                _entity_manager,
                entity_type,
                include_relations,
                format_type,
        ...
        handler_factory = operation_handlers.get(operation)
        if handler_factory is None:
            _raise_value_error(f"Unknown operation: {operation}")
