...
class DataQueryEngine(ToolBase):
    ...
    async def _search_query(
        self, entities: list[str], search_term: str, conditions: dict[str, Any] | None = None, limit: int | None = None
    ) -> dict[str, Any]:
        ...
        async def search_entity(entity_type: str):
            ...
            try:
                ...
                entity_results = await self._db_query(
                    table, filters=filters, limit=limit or 10, order_by="updated_at:desc"
                )
                ...
                return (entity_type, {"count": len(sanitized_results), "results": sanitized_results})

            except Exception as e:
                return (entity_type, {"error": str(e), "count": 0, "results": []})
    ...
    async def _aggregate_query(
        self, entities: list[str], conditions: dict[str, Any] | None = None, projections: list[str] | None = None
    ) -> dict[str, Any]:
        ...
        async def aggregate_entity(entity_type: str):
            ...
            try:
                ...
                recent_count = (
                    count_results[1] if len(count_results) > 1 and not isinstance(count_results[1], Exception) else 0
                )
                ...
                return (entity_type, {"error": str(e), "total_count": 0})
            else:
        ...
        return {"aggregation_type": "summary_stats", "entities_analyzed": entities, "results": results}

    async def _analyze_query(self, entities: list[str], conditions: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        async def analyze_entity(entity_type: str):
            try:
                ...
                if entity_type == "organization":
                    ...
                    analysis_payload = {"total_count": total_count, "analysis": "basic_metrics_only"}
        ...
        return {"analysis_type": "deep_analysis", "entities_analyzed": entities, "analysis": analysis}
    ...
    async def _analyze_organization_metrics(
        ...
        async def get_org_stats(org):
            member_task = self._db_count(
                Tables.ORGANIZATION_MEMBERS, {"organization_id": org["id"], "status": "active"}
            )
            project_task = self._db_count(Tables.PROJECTS, {"organization_id": org["id"], "is_deleted": False})
    ...
    async def _analyze_project_metrics(
        ...
        async def get_project_stats(project):
            ...
            doc_count = len(docs)
            req_tasks = [
                self._db_count(Tables.REQUIREMENTS, {"document_id": doc["id"], "is_deleted": False}) for doc in docs
            ]
    ...
    async def _relationship_query(
        self, entities: list[str], conditions: dict[str, Any] | None = None
    ) -> dict[str, Any]:
            ...
            try:
                ...
                if count > 0:
                    ...
                    samples = await self._db_query(
                        rel_table, select=select_fields, filters=filters, limit=10, order_by="created_at:desc"
                    )

                    relationships[rel_table] = {"total_count": count, "recent_samples": samples}

            except Exception as e:
                # Gracefully handle errors - don't fail entire query
                relationships[rel_table] = {"error": f"INTERNAL_SERVER_ERROR: {e!s}", "total_count": 0}
        ...
        try:
            ...
            if mode == RAGMode.SEMANTIC.value:
                ...
                search_response = await self._vector_search_service.keyword_search(
                    query=query, limit=limit, entity_types=entities, filters=conditions
                )
    ...
    try:
        ...
        async def _execute_query(selected_entities: list[str]) -> dict[str, Any]:
            if query_type == QueryType.SEARCH.value:
                ...
                return await _query_engine._search_query(selected_entities, search_term or "", conditions, limit)

            if query_type == QueryType.AGGREGATE.value:
                return await _query_engine._aggregate_query(selected_entities, conditions, projections)

            if query_type == QueryType.ANALYZE.value:
                return await _query_engine._analyze_query(selected_entities, conditions)

            if query_type == QueryType.RELATIONSHIPS.value:
                return await _query_engine._relationship_query(selected_entities, conditions)
        ...
        return {"success": False, "error": str(e), "query_type": query_type, "entities": entities}
