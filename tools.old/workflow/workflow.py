...
try:
    from ..entity.entity import _entity_manager
    from ..entity.relationship import _relationship_manager
    from ..workflow.workspace import _workspace_manager
except ImportError:
    from tools.entity.entity import _entity_manager
    from tools.entity.relationship import _relationship_manager
    from tools.workflow.workspace import _workspace_manager
...
class WorkflowExecutor(ToolBase):
    ...
    async def _setup_test_matrix_workflow(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        documents = await _entity_manager.search_entities(
            "document", filters={"project_id": project_id, "is_deleted": False}
        )

        # Then get requirements from all documents in parallel
        if documents:
            req_tasks = [
                _entity_manager.search_entities("requirement", filters={"document_id": doc["id"], "is_deleted": False})
        ...
        matrix_view_data = {
            ...
            "is_default": True,
            "configuration": {"show_coverage": True, "group_by": "priority", "filter_status": ["active"]},
        }

        matrix_view = await _entity_manager.create_entity("test_matrix_view", matrix_view_data)
    ...
    async def _bulk_status_update_workflow(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        for entity_id in entity_ids:
            task = _entity_manager.update_entity(entity_type, entity_id, {"status": new_status})
        ...
        for i, result in enumerate(update_results):
            if isinstance(result, Exception):
                results.append({"step": f"update_{entity_ids[i]}", "error": str(result)})
            elif isinstance(result, dict):
                results.append({"step": f"update_{entity_ids[i]}", "result": str(result)})
            else:
                results.append({"step": f"update_{entity_ids[i]}", "error": f"Unexpected result type: {type(result)}"})
                success_count += 1
...
async def workflow_execute(
    ...
    try:
        ...
        return {"success": False, "error": str(e), "workflow": workflow, "transaction_mode": transaction_mode}
