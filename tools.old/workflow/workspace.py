...
class WorkspaceManager(ToolBase):
    ...
    async def set_context(self, user_id: str, context_type: str, entity_id: str) -> dict[str, Any]:
        ...
        table_map = {"organization": "organizations", "project": "projects", "document": "documents"}
    ...
    async def list_workspaces(self, user_id: str, limit: int = 100, offset: int = 0) -> dict[str, Any]:
        ...
        total_count = await self._db_count(
            "organization_members", filters={"user_id": user_id, "status": "active", "is_deleted": False}
        )
        ...
        orgs = await self._db_query(
            ...
            filters={"user_id": user_id, "status": "active", "is_deleted": False},
            limit=limit,
            offset=offset,
        )
        ...
        for org_member in orgs:
            ...
            if org_data:
                ...
                project_count = await self._db_count(
                    "projects", filters={"organization_id": org_data["id"], "is_deleted": False}
                )
    ...
    async def get_smart_defaults(self, user_id: str) -> dict[str, str | None]:
        ...
        if not org_id:
            personal_org = await self._db_get_single(
                "organizations", filters={"created_by": user_id, "type": "personal", "is_deleted": False}
            )
            if personal_org:
        ...
        return {"organization_id": org_id, "project_id": project_id, "document_id": document_id}
    ...
    try:
        ...
        if operation == "get_context":
            ...
            if not context_type or not entity_id:
                _raise_missing_context_fields()
            result = await _workspace_manager.set_context(user_id, context_type, entity_id)

        elif operation == "list_workspaces":
            result = await _workspace_manager.list_workspaces(user_id, limit=limit or 100, offset=offset or 0)
        ...
        formatted_result = _workspace_manager._format_result(result, format_type)
        if isinstance(formatted_result, dict):
            hints = {key: value for key, value in context_hints.items() if value}
            if hints:
                formatted_result.setdefault("context_hints", hints)
