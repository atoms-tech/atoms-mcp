...
def create_server() -> FastMCP:
    ...
    oauth_provider = SupabaseOAuthProvider(base_url=base_url)
    if oauth_provider.is_enabled():
        oauth_provider.setup_routes(mcp)
        logger.info("OAuth Proxy Provider enabled - OAuth 2.1 endpoints available")
    else:
    ...
    def get_oauth_session(access_token: str) -> dict:
        ...
        try:
            ...
            # Create a session token for FastMCP use
            ...
            stored_session = sessions.get(session_token)
            if stored_session:
                stored_session.oauth_access_token = session.oauth_access_token
...
def main() -> None:
    """Simple CLI runner with OAuth support.
    ...
    Environment variables:
      ...
      - ATOMS_FASTMCP_BASE_URL: base URL for OAuth discovery (optional)
      - ATOMS_OAUTH_CLIENT_ID: client ID (optional)
    """
