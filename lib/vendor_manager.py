...
''')
    ...
    def create_requirements_prod(self) -> None:
        """Generate requirements-prod.txt from the uv lockfile."""
        print("📝 Exporting production requirements with uv...")
...
# ============================================================================
# Generated from pyproject.toml/uv.lock via `uv export`
# Vendored pheno-sdk packages are loaded via sitecustomize.py
# ============================================================================
...
"""
        ...
        try:
            ...
            raise RuntimeError(
                "uv export failed. Ensure uv.lock is up-to-date by running `uv lock`."
            ) from exc

        req_file.write_text(header + result.stdout)
        print(f"✅ Created requirements-prod.txt ({req_file})")
