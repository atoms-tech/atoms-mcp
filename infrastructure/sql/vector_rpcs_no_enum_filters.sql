-- Fixed Vector RPC functions WITHOUT enum filtering
-- This avoids the enum comparison issues entirely
-- Copy and paste this into your Supabase SQL Editor

-- Enable pgvector extension (run once)
CREATE EXTENSION IF NOT EXISTS vector;

-- Documents matcher (no enum filters)
...
-- Projects matcher (no enum filters)
  ...
  WHERE p.embedding IS NOT NULL
    ...
    AND (
      ...
      OR (
        ...
        -- Removed status filter to avoid enum issues
      )
    )
  ORDER BY similarity DESC
  LIMIT GREATEST(match_count, 1);
...
-- Requirements matcher (no enum filters)
...
RETURNS TABLE(
  ...
  SELECT 
    ...
    jsonb_build_object(
      ...
      'type', COALESCE(r.type::text, 'unknown'),
  ...
  WHERE r.embedding IS NOT NULL
    ...
    AND (
      ...
      OR (
        ...
        -- Removed status and priority filters to avoid enum issues
      )
    )
  ORDER BY similarity DESC
  LIMIT GREATEST(match_count, 1);
...
-- Organizations matcher (no enum filters)
...
RETURNS TABLE(
  ...
  WHERE o.embedding IS NOT NULL
    ...
    -- Removed all enum filters
  ORDER BY similarity DESC
  LIMIT GREATEST(match_count, 1);
$$;

-- Tests matcher (simplified)
...
-- Test query to verify functions work (replace with actual embedding vector)
-- SELECT 'Documents:', count(*) FROM match_documents(array_fill(0, ARRAY[768])::vector(768), 5, 0.0, null);
-- SELECT 'Projects:', count(*) FROM match_projects(array_fill(0, ARRAY[768])::vector(768), 5, 0.0, null);
