-- Fixed Vector RPC functions with proper type casting
-- Copy and paste this into your Supabase SQL Editor

-- Enable pgvector extension (run once)
CREATE EXTENSION IF NOT EXISTS vector;

-- Documents matcher (fixed)
...
-- Projects matcher (fixed with status casting)
  ...
  WHERE p.embedding IS NOT NULL
    ...
    AND (
      ...
      OR (
        ...
        AND (filters->>'status' IS NULL OR p.status::text = (filters->>'status'))
      )
    )
  ORDER BY similarity DESC
  LIMIT GREATEST(match_count, 1);
...
-- Requirements matcher (fixed with status and priority casting)
  ...
  WHERE r.embedding IS NOT NULL
    ...
    AND (
      ...
      OR (
        ...
        AND (filters->>'status' IS NULL OR r.status::text = (filters->>'status'))
        AND (filters->>'priority' IS NULL OR r.priority::text = (filters->>'priority'))
      )
    )
  ORDER BY similarity DESC
  LIMIT GREATEST(match_count, 1);
...
-- Organizations matcher (fixed with type and status casting)
...
RETURNS TABLE(
  ...
  WHERE o.embedding IS NOT NULL
    ...
    AND (
      filters IS NULL
      OR (
        (filters->>'type' IS NULL OR o.type::text = (filters->>'type'))
        AND (filters->>'status' IS NULL OR o.status::text = (filters->>'status'))
      )
    )
  ORDER BY similarity DESC
  LIMIT GREATEST(match_count, 1);
...
-- Tests matcher (simplified to avoid type issues)
...
-- Test the functions (run this after deploying to verify they work)
-- SELECT * FROM match_projects('[0,0,0...]'::vector(768), 5, 0.0, null);
