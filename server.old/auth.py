...
class RateLimitExceeded(Exception):
    ...
    def __init__(self, user_id: str, remaining: int):
        ...
        super().__init__(f"Rate limit exceeded for user {user_id}. Remaining: {remaining} requests in current window.")
...
def extract_bearer_token() -> BearerToken | None:
    """Extract bearer token from FastMCP's AuthKit OAuth or HTTP Authorization header.

    This implements a hybrid authentication approach that combines:
    - FastMCP's token verification pattern (https://gofastmcp.com/servers/auth/token-verification)
    - AuthKitProvider OAuth flow for MCP clients
    - HTTP bearer token extraction for frontend clients

    Token extraction priority:
    1. HTTP Authorization header (Bearer token) - for frontend clients
    2. FastMCP's AuthKitProvider OAuth context - for MCP clients
    3. Claims dict fallback - for backward compatibility

    This allows authenticated frontend clients to pass their AuthKit JWT
    directly via the Authorization header, enabling seamless integration
    with frontend authentication flows, while maintaining full OAuth
    support for MCP clients.
    ...
    See Also:
        - docs/deployment/FRONTEND_BEARER_TOKEN.md
        - docs/deployment/AUTHENTICATION_COMPARISON.md
        - https://gofastmcp.com/servers/auth/token-verification
    """
    # Priority 1: Check HTTP Authorization header for Bearer token
    # This enables frontend clients to pass their AuthKit JWT directly
    try:
        headers = get_http_headers()
        auth_header = headers.get("authorization", "")
        if auth_header.startswith("Bearer "):
            token_str = auth_header[7:].strip()  # Remove "Bearer " prefix
            if token_str:
                logger.debug("Using Bearer token from Authorization header")
                return BearerToken(token=token_str, source="http.authorization.bearer", claims=None)
    except Exception as e:
        # get_http_headers() may fail if not in HTTP context (e.g., stdio transport)
        logger.debug(f"Could not access HTTP headers: {e}")

    # Priority 2: Get token from FastMCP's OAuth context
    access_token = get_access_token()
    if not access_token:
        logger.debug("No access token from FastMCP OAuth or HTTP headers")
        return None

    # AuthKit tokens are stored in .token attribute
    token_str = getattr(access_token, "token", None)
    if token_str:
        logger.debug("Using AuthKit token from FastMCP OAuth")
        claims = getattr(access_token, "claims", None)
        return BearerToken(token=token_str, source="authkit.token", claims=claims if isinstance(claims, dict) else None)

    # Priority 3: Fallback - check claims dict
    claims = getattr(access_token, "claims", None)
    if isinstance(claims, dict):
        for key in ("access_token", "token", "supabase_jwt"):
            ...
            if candidate:
                ...
                return BearerToken(token=candidate, source=f"authkit.claims.{key}", claims=claims)

    logger.debug("No valid token found in access_token object or HTTP headers")
    return None


async def check_rate_limit(user_id: str, rate_limiter: RateLimiter | None = None) -> None:
...
@asynccontextmanager
async def rate_limited_operation(user_id: str, rate_limiter: RateLimiter | None = None) -> AsyncIterator[None]:
...
async def apply_rate_limit_if_configured(rate_limiter: RateLimiter | None = None) -> BearerToken | None:
