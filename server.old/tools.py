...
def register_workspace_tool(
    mcp: FastMCP, workspace_operation: Callable, rate_limiter: RateLimiter | None = None
) -> None:
    ...
    async def workspace_tool(inputs: WorkspaceToolInput) -> dict[str, Any]:
...
def register_entity_tool(mcp: FastMCP, entity_operation: Callable, rate_limiter: RateLimiter | None = None) -> None:
...
def register_relationship_tool(
    mcp: FastMCP, relationship_operation: Callable, rate_limiter: RateLimiter | None = None
) -> None:
...
def register_workflow_tool(mcp: FastMCP, workflow_execute: Callable, rate_limiter: RateLimiter | None = None) -> None:
...
def register_query_tool(mcp: FastMCP, data_query: Callable, rate_limiter: RateLimiter | None = None) -> None:
