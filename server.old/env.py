...
def temporary_env(**kwargs: str) -> Iterator[None]:
    ...
    original: dict[str, str | None] = {key: os.environ.get(key) for key in kwargs}
    ...
    try:
        ...
        for key, env_value in original.items():
            if env_value is None:
                os.environ.pop(key, None)
            else:
                # Type narrowing: env_value is guaranteed to be str (not None) after the check
...
def get_fastmcp_vars() -> dict[str, str]:
    ...
    Examples:
        ...
        ...     print(f"{key}={value}")
    """
    return {k: v for k, v in os.environ.items() if "FASTMCP" in k.upper()}
