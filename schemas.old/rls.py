...
async def user_can_access_project(project_id: str, user_id: str, db_adapter) -> bool:
    ...
    try:
        ...
        project_member = await db_adapter.get_single(
            Tables.PROJECT_MEMBERS, filters={"project_id": project_id, "user_id": user_id, "is_deleted": False}
        )
        if project_member:
            return True

        # Get project to check visibility and organization
        project = await db_adapter.get_single(Tables.PROJECTS, filters={"id": project_id, "is_deleted": False})
...
async def is_project_owner_or_admin(project_id: str, user_id: str, db_adapter) -> bool:
    ...
    try:
        ...
        logger.exception("Error checking project ownership")
        return False
    else:
...
async def is_super_admin(user_id: str, db_adapter) -> bool:
    ...
    try:
        ...
        await db_adapter.get_single(
            Tables.ORGANIZATION_MEMBERS,
            filters={"user_id": user_id, "role": UserRoleType.SUPER_ADMIN.value, "is_deleted": False},
        )
    except Exception:
        logger.exception("Error checking super admin status")
        return False
    else:
        return True
...
async def get_user_organization_ids(user_id: str, db_adapter) -> list[str]:
    ...
    try:
        memberships = await db_adapter.query(
            Tables.ORGANIZATION_MEMBERS, filters={"user_id": user_id, "is_deleted": False}
        )

        return [m["organization_id"] for m in memberships if "organization_id" in m]

    except Exception:
        logger.exception("Error getting user organizations")
        return []
...
async def is_organization_owner_or_admin(org_id: str, user_id: str, db_adapter) -> bool:
    ...
    try:
        member = await db_adapter.get_single(
            Tables.ORGANIZATION_MEMBERS, filters={"organization_id": org_id, "user_id": user_id, "is_deleted": False}
        )
        ...
        logger.exception("Error checking organization ownership")
        return False
    else:
    ...
    async def validate_select(self, record: dict[str, Any]):
        ...
        if not await self.can_select(record):
            raise PermissionDeniedError(
                self.__class__.__name__.replace("Policy", ""), "SELECT", "User does not have read access to this record"
            )
...
class OrganizationPolicy(TablePolicy):
    ...
    async def can_insert(self, _data: dict[str, Any]) -> bool:
        """Authenticated users can create organizations."""
        return bool(self.user_id)

    async def can_update(self, record: dict[str, Any], _data: dict[str, Any]) -> bool:
...
class ProjectPolicy(TablePolicy):
    ...
    async def can_delete(self, record: dict[str, Any]) -> bool:
        ...
        member = await self.db_adapter.get_single(
            Tables.PROJECT_MEMBERS, filters={"project_id": project_id, "user_id": self.user_id, "is_deleted": False}
        )
...
class RequirementPolicy(TablePolicy):
    ...
    async def _get_project_id(self, record: dict[str, Any]) -> str | None:
        ...
        doc = await self.db_adapter.get_single(Tables.DOCUMENTS, filters={"id": doc_id, "is_deleted": False})
...
class ProfilePolicy(TablePolicy):
    ...
    async def can_select(self, _record: dict[str, Any]) -> bool:
    ...
    async def can_delete(self, _record: dict[str, Any]) -> bool:
