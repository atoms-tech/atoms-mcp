...
class SchemaSync:
    ...
    def regenerate_schemas(self) -> bool:
        ...
        cmd = [
            ...
            "postgres",
            "--dir",
            str(self.schemas_dir / "generated"),
            "--singular-names",
            "--schema",
            "public",
        ]
        ...
        for table_name, columns in aliased_enums.items():
            for column_name, enum_name in columns.items():
                ...
                for suffix in ["BaseSchema", "Insert", "Update"]:
                    ...
                    def replace_in_class(match, enum_name=enum_class_name):
                        return match.group(1) + match.group(2) + enum_name + match.group(3) + match.group(4)
...
def main():
    ...
    parser.add_argument("--check", action="store_true", help="Check for schema drift (exit 1 if drift found)")
    parser.add_argument("--diff", action="store_true", help="Show detailed differences")
    parser.add_argument(
        "--regenerate", action="store_true", help="Regenerate schemas from database using supabase-pydantic"
    )
    parser.add_argument("--update", action="store_true", help="Update local schema files (alias for --regenerate)")
    parser.add_argument("--report", action="store_true", help="Generate markdown report")
    parser.add_argument("--project-id", type=str, help="Supabase project ID (default: from env)")
